...
static BOOL marker_matches(const wchar_t *path) {
    ...
    CloseHandle(file);
    return ok && strncmp(value, "0.3.4", 5) == 0;
}

static BOOL write_marker(const wchar_t *path) {
    HANDLE file = CreateFileW(path, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (file == INVALID_HANDLE_VALUE) return FALSE;
    static const char value[] = "0.3.4\r\n";
    DWORD written = 0;
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE previous, PWSTR command_line, int show_command) {
    ...
    HANDLE setup_mutex = CreateMutexW(NULL, TRUE, L"Local\\OverheadLink-Setup-v0.3.4");
